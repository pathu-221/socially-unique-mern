...
import { CreatePostsDto, UpdatePostsDto } from "../dto/create-posts.dto";
...
router.put('/:postId', authenticate, async (req: IRequest, res: Response) => {
    ...
    try {
        ...
        const post = await Posts.findOne({ _id: postId }).populate('user', "_id username");

        if (!post) res.send({ status: 0, msg: 'Post not Found!' });

        if (post.user._id !== new ObjectId(req.user._id)) res.send({ status: 0, msg: 'This post is not yours!' });
        ...
        const updatedPost = Posts.updateOne({ _id: postId }, {
            ...updatePostDto,
            picture: url
        }) 
